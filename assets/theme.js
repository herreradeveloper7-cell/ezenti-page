...
var Delegate = /*@__PURE__*/getDefaultExportFromCjs$1(browser.exports);
...
var preventDefault$2 = function preventDefault(rawEvent) {
...
var disableBodyScroll = function disableBodyScroll(targetElement, options) {
  ...
  if (isIosDevice) {
    ...
    if (!documentListenerAdded) {
      document.addEventListener('touchmove', preventDefault$2, hasPassiveEvents ? { passive: false } : undefined);
...
var enableBodyScroll = function enableBodyScroll(targetElement) {
  ...
  if (isIosDevice) {
    ...
    if (documentListenerAdded && locks.length === 0) {
      document.removeEventListener('touchmove', preventDefault$2, hasPassiveEvents ? { passive: false } : undefined);
...
function getSizedImageUrl(src, size) {
  ...
  var match = src.match(/\.(jpg\.webp|jpeg\.webp|jpg|jpeg|gif|png|bmp|bitmap|tiff|tif|webp|svg)(\?v=\d+)?$/i);
  if (match) {
...
const selectors$17 = {
  ...
  quantityInput: ".product__input[data-quantity-input]",
...
const selectors$$ = {
  slider: "[data-slider]",
  lightboxSlide: "[data-slide] .media--has-lightbox a",
  prev: "[data-prev]",
  next: "[data-next]"
};
var productCarousel = node => {
  const sliderElement = n$2(selectors$$.slider, node);
  const lightboxSlides = t$3(selectors$$.lightboxSlide, sliderElement);
  ...
  const config = {
    ...
    on: {
      ready: function () {
        ...
        setTimeout(() => {
          _updateNavigationVisibility(this.selectedIndex);
  ...
  const _updateNavigationVisibility = currentSlide => {
  ...
  import(flu.chunks.flickity).then(({
    ...
    slider.on("change", index => {
      _updateNavigationVisibility(index);
    });
  });
  let prevClick;
  let nextClick;
...
const classes$f = {
  ...
  isMobileThumbnailStyle: "product--mobile-gallery-style-thumbnails",
...
var AnimateProduct = node => {
  ...
  if (shouldAnimate(node)) {
    ...
    } else if (a$1(node, classes$f.isThumbnailStyle) || a$1(node, classes$f.isMobileThumbnailStyle)) {
      if (featuredImage) {
  ...
  constructor(node, options = {}) {
    ...
    this.gsapImport = import(flu.chunks.gsap).then(({
      ...
      this.splitItems = new SplitText(this.node, {
        type: "lines",
        linesClass: "animation-fade-up-split-reveal__line split-text-line",
        lineThreshold: 0.5
      });
...
function QuickAddProductForm(node, openQuickCart) {
  ...
  function _onFormSubmit(e) {
    ...
    cart.addItem(formElement).then(({
      ...
      // Shopify returns a different error message depending on whether the quantity
      // you're trying to add exceeds the available stock, or you've already got the
      // remaining stock in your cart. As such, we want to surface a different message
      // for each scenario.
      //
      //  - 'stock' error type: quantity exceeds available stock
      //  - 'cart' error type: remaining stock is already in the cart
      if (error.message.includes("can't add more")) {
        quantityError.setAttribute("data-error-type", "stock");
      } else {
        quantityError.setAttribute("data-error-type", "cart");
      }
...
function renderLineProperties(properties) {
  ...
  return r(Boolean(Object.keys(properties).length), Object.keys(properties).reduce((markup, title) => {
    if (title.charAt(0) !== "_") {
      return markup + `<div class="quick-cart__item-properties">${properties[title] ? `${title}: ` : ``}${properties[title]}</div>`;
    } else {
      return markup;
    }
  }, ""));
...
function renderCartDiscounts(discounts) {
  return r(Boolean(discounts.length), `
    ...
    return `<div>${discount.title} (-${formatMoney(discount.total_allocated_amount)})</div>`;
  })}
      </ul>
    `);
}
...
register("header", {
  onLoad() {
    ...
    function updateCartCount(item_count) {
    ...
    // Listen for custom event that apps can use to refresh the cart/cart count
    e$3(document, "apps:product-added-to-cart", () => {
      if (enableQuickCart) return;
      cart.get().then(cart => {
        updateCartCount(cart.item_count);
      });
    }), e$3(menuButtons, "click", preventDefault$1(menu$1.open)), e$3(searchButtons, "click", preventDefault$1(quickSearch.open)), e$3(document, "visibilitychange", () => {
    ...
    if (enableQuickCart) {
      this.listeners.push(e$3(cartIcon, "click", preventDefault$1(quickCart.open)));
    }
...
function Popup(container) {
  ...
  let bodyLeave = () => {};
  const mouseleave = e => {
    if (!e.relatedTarget && !e.toElement && !window.Shopify.designMode) {
      showPopup();
      bodyLeave();
    }
  };
...
register("featured-product", {
  ...
  onFormSubmit(e) {
    ...
    cart.addItem(this.formElement).then(({
      ...
      //  - 'cart' error type: remaining stock is already in the cart
      if (error.message.includes("can't add more")) {
        this.quantityError.setAttribute("data-error-type", "stock");
      } else {
        this.quantityError.setAttribute("data-error-type", "cart");
      }
...
register("video-hero", {
  ...
  onLoad() {
    ...
    // Use px instead of default vh for height vars to avoid CLS within section previews
    if (window.Shopify.visualPreviewMode === true) {
      u$1(this.container, "visual-preview");
      setScreenHeightVars(this.container);
    }
    if (video.length) {
  ...
  _revealSlide(slide) {
    ...
    this.gsapImport.then(({
      ...
      const quoteTextlines = new SplitText(quoteText, {
        type: "lines",
        linesClass: "quote__item-text-lines split-text-line",
        lineThreshold: 0.5
      });
...
register("product", {
  ...
  onLoad() {
    ...
    this._initEvents();
    if (n$2(".product__media-slider", this.container)) {
      this.productCarousel = productCarousel(this.container);
    }
  ...
  onOptionChange({
    ...
    if (variant.featured_media) {
      ...
      if (!isMobile$1() && this.galleryStyle !== "thumbnails" && this.galleryStyle !== "slider") {
      ...
      // Switch slide when slider
      if (this.mobileGalleryStyle === "slider" || this.galleryStyle === "slider") {
        this.productCarousel.select(variant.featured_media.position - 1);
      }
    }
...
const filtering = container => {
  ...
  /**
   * Note: This function never ends up running because when filters are applied, it's
   * always triggered by the "Apply" button in the filter drawer, meaning there's never
   * a target passed in.
   *
  ...
  function syncForms(target) {
    ...
    targetInputs.forEach(targetInput => {
      ...
      if (targetInput.type === "checkbox" || targetInput.type === "radio") {
        const items = t$3(`input[data-name-escaped='${nameEscaped}'][data-value-escaped='${valueEscaped}']`);
        items.forEach(input => {
          input.checked = targetInput.checked;
        });
      } else {
        const items = t$3(`input[data-name-escaped='${nameEscaped}']`);
        items.forEach(input => {
...
window.SHA = "92723627a8";
if (!sessionStorage.getItem("flu_stat_recorded") && !window.Shopify?.designMode) {
